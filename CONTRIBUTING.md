...
packages/sf-game-data/data/
  stable/        en-US.json + meta.json + sf-game-data.json
  experimental/  en-US.json + meta.json + sf-game-data.json
```
...
> `sf-game-data.json` must agree on **both** `gameVersion` and `build`. You may
> **not** hand-edit the version/build in either file — regenerate
> `sf-game-data.json` with the extractor (which stamps the version you pass it)
...
4. Commit **only** those three files, e.g.:
   ...
   git add packages/sf-game-data/data/stable/en-US.json \
           ...
           packages/sf-game-data/data/stable/sf-game-data.json
