...
- **`world-locations.json`** — the extracted collectible/resource-node dataset
  ...
  [`packages/sf-game-data/extract`](./packages/sf-game-data/extract/README.md).
...
> files under a single `packages/sf-game-data/data/<channel>/` directory (no code,
...
1. Copy `CommunityResources/Docs/en-US.json` from a game install on the relevant
   channel to `packages/sf-game-data/data/<channel>/en-US.json` (it is UTF-16 LE —
...
2. Write `packages/sf-game-data/data/<channel>/meta.json` with the `gameVersion`,
   `build` (the in-game build number) and `channel`.
3. **Regenerate** `packages/sf-game-data/data/<channel>/world-locations.json` by
   running the extractor against that build — see
   [`packages/sf-game-data/extract/README.md`](./packages/sf-game-data/extract/README.md). Pass
...
4. Commit **only** those three files, e.g.:
   ...
   git add packages/sf-game-data/data/stable/en-US.json \
           packages/sf-game-data/data/stable/meta.json \
           packages/sf-game-data/data/stable/world-locations.json
...
> Only the bundled channel copies under `packages/sf-game-data/data/` are tracked; a local
