 ...
 *   3. Bundled channel — committed game data under `packages/mcp/data/<channel>/`,
...
export function resolveDocsPath(
  ...
  warnings.push(
    `No game data available: set SATISFACTORY_DOCS_PATH or SATISFACTORY_GAME_DIR, or add bundled data under packages/mcp/data/{${GAME_CHANNELS.join(',')}}/. Starting with empty game data.`,
  );
